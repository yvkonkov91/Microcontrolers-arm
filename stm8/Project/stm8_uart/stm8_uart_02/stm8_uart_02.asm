...
; This file was generated Thu May 10 21:41:22 2018
;--------------------------------------------------------
	.module stm8_uart_02
	.optsdcc -mstm8
...
;	../lib/lib1/inc/stm8s_uart.h: 12: UART1->CR1  |= 0b00000000;   //установка колличества бит передачи 8/9
	...
	ldw	x, #0x5234
	ld	(x), a
;	../lib/lib1/inc/stm8s_uart.h: 13: UART1->CR2  |= 0b01001100;   //разрешение работы RX & TX
	ldw	x, #0x5235
	ld	a, (x)
	or	a, #0x4c
	ld	(x), a
...
;	../lib/lib1/inc/stm8s_uart.h: 44: uart_tx_byte(temp%10+'0');
	...
	addw	sp, #3
	ret
;	../lib/lib1/inc/stm8s_uart.h: 48: unsigned char uart_rx_byte(void)  //  Получение байта
;	-----------------------------------------
;	 function uart_rx_byte
;	-----------------------------------------
_uart_rx_byte:
;	../lib/lib1/inc/stm8s_uart.h: 50: while(!(UART1->SR&(1<<7)))    //  Устанавливается, когда регистр свободен
00101$:
	ldw	x, #0x5230
	ld	a, (x)
	tnz	a
	jrpl	00101$
;	../lib/lib1/inc/stm8s_uart.h: 52: return UART1->DR;
	ldw	x, #0x5231
	ld	a, (x)
	ret
;	./stm8_uart_02.c: 6: void pin_cfg()  //функция настройки GPIO
;	-----------------------------------------
;	 function pin_cfg
;	-----------------------------------------
_pin_cfg:
;	./stm8_uart_02.c: 8: GPIOB->DDR |= 0b00100000;   //устанавливаем на выход пин 5 порта B
	ldw	x, #0x5007
	ld	a, (x)
	or	a, #0x20
	ld	(x), a
;	./stm8_uart_02.c: 9: GPIOB->CR1 |= 0b00100000;   //устанавливаем подтяжку пин 5 порта B
	ldw	x, #0x5008
	ld	a, (x)
	or	a, #0x20
	ld	(x), a
;	./stm8_uart_02.c: 10: GPIOB->CR2 |= 0b00000000;   //минимальная скорость на выход / без прерывания на вход
	ldw	x, #0x5009
	ld	a, (x)
	ldw	x, #0x5009
	ld	(x), a
;	./stm8_uart_02.c: 12: GPIOD->DDR |= 0b00100000;   //устанавливаем на выход пин 5 порта D TX
	ldw	x, #0x5011
	ld	a, (x)
	or	a, #0x20
	ld	(x), a
;	./stm8_uart_02.c: 13: GPIOD->CR1 |= 0b00000000;   //без подтяжки
	ldw	x, #0x5012
	ld	a, (x)
	ldw	x, #0x5012
	ld	(x), a
;	./stm8_uart_02.c: 14: GPIOD->CR2 |= 0b00000000;   //без прерывания на вход / минимальная скорость на выход
	ldw	x, #0x5013
	ld	a, (x)
	ldw	x, #0x5013
	ld	(x), a
	ret
;	./stm8_uart_02.c: 18: extern void uart1_interrupt(void) __interrupt(18)   // Обработчик прерываний по приему UART
;	-----------------------------------------
;	 function uart1_interrupt
;	-----------------------------------------
_uart1_interrupt:
;	./stm8_uart_02.c: 29: if((UART1->SR & (1<<6))!=0)
	ldw	x, #0x5230
	ld	a, (x)
	bcp	a, #0x40
	jreq	00103$
;	./stm8_uart_02.c: 32: UART1->SR ^= (1<<6);  
	ldw	x, #0x5230
	ld	a, (x)
	xor	a, #0x40
	ld	(x), a
00103$:
	iret
;	./stm8_uart_02.c: 39: int main( void )
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
;	./stm8_uart_02.c: 41: pin_cfg();
	call	_pin_cfg
;	./stm8_uart_02.c: 43: clk_cfg();
	call	_clk_cfg
;	./stm8_uart_02.c: 45: tim4_cfg();
	call	_tim4_cfg
;	./stm8_uart_02.c: 47: uart1_cfg();
	call	_uart1_cfg
;	./stm8_uart_02.c: 49: rim();  //разрешаем глобальное прерывание
	rim 
00102$:
;	./stm8_uart_02.c: 55: GPIOB->ODR ^= 0b00100000;
	ldw	x, #0x5005
	ld	a, (x)
	xor	a, #0x20
	ld	(x), a
;	./stm8_uart_02.c: 56: delay_ms(5000);
	push	#0x88
	push	#0x13
	call	_delay_ms
	addw	sp, #2
